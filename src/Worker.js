...
async function schema(env) {
  ...
  const q = [
    ...
    // ⭐ نظام الحسابات — طبقة ثانية فوق كلمة مرور البوابة المشتركة: بوابة واحدة تفتح الوصول،
    // ثم كل شخص يُنشئ حسابه الخاص (اسم مستخدم + كلمة مرور) ليُعزَل عن بقية المستخدمين.
    'CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, username TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, password_salt TEXT NOT NULL, created_at INTEGER NOT NULL)',
    'CREATE TABLE IF NOT EXISTS user_login_attempts (username TEXT PRIMARY KEY, attempts INTEGER NOT NULL DEFAULT 0, locked_until INTEGER, updated_at INTEGER NOT NULL)',
  ];
  for (const sql of q) {
  ...
  const migrations = [
    ...
    // ⭐ عزل البيانات بين المستخدمين: الملكية تُحفَظ على مستوى المحادثة فقط (وليس في كل جدول
    // فرعي) — كل سجل آخر (رسائل، مرفقات، تجارب...) يُستدل على مالكه عبر chat_id الذي ينتمي
    // إليه، فيكفي فرض التحقق من الملكية عند كل chatId في نقطة واحدة مركزية بدل تكرارها.
...
// ============ دوال مساعدة ============
async function getSessionRow(env, token) {
...
async function isSessionValid(env, token) {
  return !!(await getSessionRow(env, token));
}

// ⭐ حساب المستخدم — PBKDF2 عبر Web Crypto (متاحة أصلاً في بيئة Workers، بلا مكتبات خارجية)
...
// ⭐ مقارنة بوقت ثابت — تمنع تسريب معلومات عن مدى تطابق التجزئة عبر فروقات التوقيت
...
// ⭐ يُستدعى مباشرة بعد isSessionValid في نقطة التوزيع المركزية — يُعيد أيضاً هوية المستخدم
// المرتبطة بالجلسة (إن وُجدت) ليُستخدَم لاحقاً في التحقق من ملكية كل chatId.
async function resolveSession(env, token) {
  if (!env.ACCESS_PASSWORD) return { gateOk: true, userId: null }; // البوابة مُعطَّلة كلياً عبر إعدادات البيئة
  const row = await getSessionRow(env, token);
  if (!row) return { gateOk: false, userId: null };
  return { gateOk: true, userId: row.user_id || null };
}

async function userSignup(env, b) {
  const username = String(b.username || '').trim().slice(0, 40);
  const password = String(b.password || '');
  if (!/^[a-zA-Z0-9_\u0600-\u06FF]{3,40}$/.test(username)) {
    return json({ error: 'اسم المستخدم يجب أن يكون 3-40 حرفاً (حروف/أرقام/شرطة سفلية فقط)' }, 400);
  }
  if (password.length < 6) return json({ error: 'كلمة المرور يجب ألا تقل عن 6 أحرف' }, 400);

  const existing = await env.DB.prepare('SELECT id FROM users WHERE username = ?').bind(username).first();
  if (existing) return json({ error: 'اسم المستخدم مستخدَم بالفعل' }, 409);
  ...
  // ⭐ يربط الحساب الجديد بنفس جلسة البوابة الحالية مباشرة — لا حاجة لتسجيل دخول منفصل بعد الإنشاء
  if (b.sessionToken) {
    await env.DB.prepare('UPDATE sessions SET user_id = ? WHERE token = ?').bind(userId, b.sessionToken).run();
  }
  return json({ success: true, userId, username });
}

async function userLogin(env, b) {
  const username = String(b.username || '').trim().slice(0, 40);
  const password = String(b.password || '');
  const nowMs = Date.now();

  const lockCheck = await env.DB.prepare('SELECT attempts, locked_until FROM user_login_attempts WHERE username = ?').bind(username).first();
  if (lockCheck?.locked_until && lockCheck.locked_until > nowMs) {
    const remainingMin = Math.ceil((lockCheck.locked_until - nowMs) / 60000);
    return json({ error: `تم حظر هذا الحساب مؤقتاً. حاول بعد ${remainingMin} دقيقة.`, locked: true }, 429);
  }
  ...
  if (!ok) {
    ...
    await env.DB.prepare(
      `INSERT INTO user_login_attempts (username, attempts, locked_until, updated_at) VALUES (?, ?, ?, ?)
       ON CONFLICT(username) DO UPDATE SET attempts = ?, locked_until = ?, updated_at = ?`
    ).bind(username, currentAttempts, lockedUntil, nowMs, currentAttempts, lockedUntil, nowMs).run();
    return json({ error: shouldLock ? 'بيانات خاطئة. تم حظر الحساب 10 دقائق.' : `بيانات خاطئة. المتبقي: ${5 - currentAttempts}` }, 401);
  }

  await env.DB.prepare(
    `INSERT INTO user_login_attempts (username, attempts, locked_until, updated_at) VALUES (?, 0, NULL, ?)
     ON CONFLICT(username) DO UPDATE SET attempts = 0, locked_until = NULL, updated_at = ?`
  ).bind(username, nowMs, nowMs).run();

  if (b.sessionToken) {
    await env.DB.prepare('UPDATE sessions SET user_id = ? WHERE token = ?').bind(user.id, b.sessionToken).run();
  }
  return json({ success: true, userId: user.id, username });
}

// ⭐ نقطة تحقق مركزية واحدة: هل chatId هذا يخص userId؟ محادثة غير موجودة بعد تُعتبر مسموحة
// (ستُنشأ لهذا المستخدم عند أول رسالة فيها) — هذا يغني عن إضافة user_id لكل جدول فرعي،
// بما أن كل سجل آخر (رسائل، مرفقات، تجارب...) يُستدل على مالكه عبر chat_id فقط.
async function assertChatOwnership(env, chatId, userId) {
  if (!chatId) return true;
  const row = await env.DB.prepare('SELECT user_id FROM conversations WHERE id = ?').bind(chatId).first();
  if (!row) return true; // محادثة جديدة — ستُنشأ لهذا المستخدم
  return !row.user_id || row.user_id === userId; // محادثات قديمة بلا مالك (قبل هذا التحديث) تبقى قابلة للوصول
}
...
// ============ persist / load ============
async function persistMessage(env, chatId, role, content, modelId = null, attachmentRef = null, userId = null) {
  const t = Date.now();
  ...
  // ⭐ user_id يُكتب فقط عند إنشاء المحادثة لأول مرة (ON CONFLICT لا يلمسه) — يضمن أن
  // المحادثة تبقى مملوكة لمن أنشأها فعلاً حتى لو استدعى الخادم persistMessage لاحقاً بلا userId
  await env.DB.prepare('INSERT INTO conversations (id,title,user_id,created_at,updated_at) VALUES (?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET updated_at=?').bind(chatId, textContent.slice(0, 80) || 'محادثة جديدة', userId, t, t, t).run();
}
...
async function chat(env, b) {
  ...
  const stream = sseStream(async (send, controller) => {
    ...
    await persistMessage(env, b.chatId, 'user', userMessageContent, null, primaryAttachmentRef, b.userId || null);
...
async function searchAll(env, b) {
  ...
  // ⭐ نطاق ملكية موحّد — أي بحث لا يحدّد chatId يجب أن يبقى محصوراً بمحادثات هذا المستخدم
  // فقط (أو المحادثات القديمة بلا مالك بعد)، وإلا يتسرّب محتوى مستخدمين آخرين عبر البحث
  const ownedScope = 'chat_id IN (SELECT id FROM conversations WHERE user_id = ? OR user_id IS NULL)';
  const conversations = chatId ? { results: [] } : await env.DB.prepare('SELECT id,title,updated_at FROM conversations WHERE title LIKE ? AND (user_id = ? OR user_id IS NULL) ORDER BY updated_at DESC LIMIT 30').bind(like, b.userId || null).all();
  const msgs = chatId
    ...
    : await env.DB.prepare(`SELECT id,chat_id,role,content,created_at FROM messages WHERE content LIKE ? AND ${ownedScope} ORDER BY created_at DESC LIMIT 50`).bind(like, b.userId || null).all();
  const files = chatId
    ...
    : await env.DB.prepare(`SELECT id,chat_id,name,mime,size,text_preview,created_at FROM attachments WHERE (name LIKE ? OR text_preview LIKE ?) AND ${ownedScope} ORDER BY created_at DESC LIMIT 50`).bind(like, like, b.userId || null).all();
...
async function listConversations(env, b) {
  ...
  const r = await env.DB.prepare(`SELECT c.id,c.title,c.updated_at,
    ...
    FROM conversations c WHERE c.user_id = ? OR c.user_id IS NULL ORDER BY c.updated_at DESC LIMIT ?`).bind(b.userId || null, limit).all();
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET') {
      ...
      // ⭐ PWA: manifest.json وservice-worker.js وأيقونات icons/* لم تكن تُخدَّم إطلاقاً —
      // فقط '/' و'/index.html' كانا مُوجَّهَين لـ ASSETS، وأي مسار آخر كان يسقط على استجابة
      // JSON عامة بدل محتوى الملف الفعلي. الآن أي ملف ثابت حقيقي في مجلد الأصول يُخدَّم مباشرة.
      if (env.ASSETS && /^\/(manifest\.json|service-worker\.js|icons\/.+)$/.test(pathname)) {
    ...
    try {
      ...
      const sessionInfo = await resolveSession(env, b.sessionToken);
      if (!sessionInfo.gateOk) return json({ error: 'الجلسة غير صالحة', authRequired: true }, 401);

      // ⭐ الحساب الشخصي (الطبقة الثانية فوق بوابة كلمة المرور المشتركة) — يُسمح بهذين
      // الإجراءين حتى بلا حساب بعد، لأنهما وسيلة الحصول على واحد
      if (b.action === 'user_signup') return userSignup(env, b);
      if (b.action === 'user_login') return userLogin(env, b);

      if (!sessionInfo.userId) return json({ error: 'يلزم تسجيل الدخول بحسابك الشخصي', needsUserLogin: true }, 401);
      b.userId = sessionInfo.userId; // ⭐ يُقرأ لاحقاً داخل chat()/listConversations() وغيرها لعزل البيانات

      // ⭐ نقطة تحقق واحدة تغطي كل الإجراءات التي تحمل chatId — تمنع أي شخص من الوصول
      // لمحادثة شخص آخر حتى لو عرف معرّفها، بغض النظر عن الإجراء المطلوب تحديداً
      if (b.chatId && !(await assertChatOwnership(env, b.chatId, b.userId))) {
        return json({ error: 'هذه المحادثة لا تخصّ حسابك' }, 403);
      }
